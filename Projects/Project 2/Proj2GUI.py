...
class PointLineView( QWidget ):
	...
	def clearPoints(self, points=None):
		if not points:
			self.pointList = {}
		else:
			for color in self.pointList:
				for pt in points:
					try:
						self.pointList[color].remove(pt)
					except:
						pass
...
# and a view object so the GUI can be updated
	...
	def untilBroken(self):
		for i in range(5):
			self.generateClicked()
			self.solveClicked()
			self.randSeed.setText(str(int(self.randSeed.text())+1))
...
# Setting up the GUI
	def initUI( self ):
		...
		self.breakButton    = QPushButton('Run \'till Broken')
		...
		self.breakButton.clicked.connect(self.untilBroken)
